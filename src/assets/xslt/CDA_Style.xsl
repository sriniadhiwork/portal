      ...
      <table class="header_table">
        <tbody>
          <tr>
            <td width="20%" bgcolor="#66ccff">
              <span class="td_label">
                <xsl:text>Care Members:</xsl:text>
              </span>
            </td>
          </tr>
          ...
          <xsl:if test="not(n1:id/@nullFlavor)">
            <tr>
              ...
              <td >
                ...
                -->
                <xsl:for-each select="n1:patient/n1:languageCommunication">
                  <xsl:call-template name="show-lang">
...
<!--Show Lang-->
<xsl:template name="show-lang">
  <xsl:param name ="languageCode"/>
  <xsl:choose>
    <xsl:when test="$languageCode/@code">
      <xsl:value-of select="$languageCode/@displayName"/>
      <xsl:text>(</xsl:text>
      <xsl:value-of select="$languageCode/@code"/>
      <xsl:text>) </xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>No Preferred Language</xsl:text>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
  ...
  <xsl:choose>
    <xsl:when test="$asgnEntity/n1:assignedPerson/n1:name">
      ...
      <xsl:if test="$asgnEntity/n1:addr">
        <xsl:call-template name="show-address">
          <xsl:with-param name="address" select="$asgnEntity/n1:addr"/>
        </xsl:call-template>
      </xsl:if>

      <xsl:if test="$asgnEntity/n1:telecom">
        <xsl:call-template name="show-telecom">
          <xsl:with-param name="telecom" select="$asgnEntity/n1:telecom"/>
        </xsl:call-template>
      </xsl:if>
    </xsl:when>
    <xsl:otherwise>
...
<!-- show assignedEntity -->
<xsl:template name="show-careteam">
  <xsl:param name="asgnEntity"/>
  <xsl:choose>
    <xsl:when test="$asgnEntity/n1:assignedPerson/n1:name">
      <xsl:call-template name="show-name">
      ...
      <xsl:if test="$asgnEntity/n1:telcom">
        <xsl:call-template name="show-telecom">
          <xsl:with-param name="telecom" select="$asgnEntity/n1:telecom"/>
        </xsl:call-template>
      </xsl:if>
