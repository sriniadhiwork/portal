(function() {
    ...
    describe('portal.aiLocationStatus', function() {
        ...
        mock.chart = {type:'PieChart',options:{title:'San Samon Regional Medical Center (average response time: 78s)',is3D:true},data:{cols:[{id:'s',label:'Status',type:'string'},{id:'c',label:'Count',type:'number'}],rows:[{c:[{v:'Success'},{v:1}]},{c:[{v:'Failed'},{v:0}]},{c:[{v:'Cancelled'},{v:0}]}]}}
        ...
        it('should load the queried Locations on load', function () {
            expect(vm.locationStatistics.length).toBe(3);
        });
